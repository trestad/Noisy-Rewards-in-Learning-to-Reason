...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch, is_test=False, add_format_reward_when_wrong=False):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            format_valid = micro_batch['format_valid']
            ...
            if self.use_remove_padding:
                ...
                # print(f'RM score shape: {rm_score.shape}, line 1041 fsdp_workers.py')

            # extract the result of the last valid token
            eos_mask_idx = torch.argmax(position_ids * attention_mask, dim=-1)  # (bsz,)
            rm_score = rm_score[torch.arange(batch_size), eos_mask_idx]
            rm_score *= format_valid
            # print(f'RM score shape: {rm_score.shape}, line 1057 fsdp_workers.py')
            
            if not is_test and add_format_reward_when_wrong:
                rm_score = torch.where(rm_score < 0.5, torch.clamp(rm_score + 0.1 * micro_batch['rm_think_reward'], max=1.0), rm_score)
                print(f'RM score: {rm_score}, Format valid: {format_valid} line 1061 fsdp_workers.py')
            return rm_score
    ...
    def think_reward(self, solution_str):
        ...
        score += float("1. " in solution_str)
        score += float("2. " in solution_str)
        score += float("3. " in solution_str)
        score += float("4. " in solution_str)
        score += float("step 1 " in solution_str)
        score += float("step 2 " in solution_str)
        score += float("step 3. " in solution_str)
        score += float("step 4 " in solution_str)
        score += float('considering the' in solution_str)
        score += float("i'll" in solution_str)
        score += float("i will" in solution_str)
        score += float("i would" in solution_str)
        score += float("i should" in solution_str)
        score /= 20
        score = min(1, score)
    ...
    def extract_think_and_answer(self, solution_str):      
        think_start = solution_str.rfind("<think>")
        think_end = solution_str.rfind("</think>")
        ans_start = solution_str.rfind("<answer>")
        ans_end = solution_str.rfind("</answer>")
        
        if think_start != -1 and think_end != -1 and ans_start != -1 and ans_end != -1 and \
            (think_end > think_start + len("<think>")) and (ans_end > ans_start + len("<answer>")) \
            and think_end < ans_start:
                think = solution_str[think_start + len("<think>"): think_end]
                answer = solution_str[ans_start + len("<answer>"): ans_end]
                return think, answer
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            # extract raw prompt
            chat: list = copy.deepcopy(data.non_tensor_batch['raw_prompt'][i])
            # extract response
            ...
            think_process, answer = self.extract_think_and_answer(response)
            
            if think_process is not None and answer is not None:  
                think_r = self.think_reward(think_process + answer)
                rm_think_reward.append(think_r)
                format_valid.append(True)
            else:
                think_r = None
                rm_think_reward.append(0.0)
                format_valid.append(False)
                answer = 'No valid answer is extracted'

            
            if self.rank == 0:
                print("==" * 10 + 'start' + "==" * 10)
                print("[Question]: ", chat[-1]['content'])
                print("[Think Process]: ", think_process)
                print("[Think Reward]: ", think_r)
                print("[Answer]: ", answer)
            
            chat.append({'role': 'assistant', 'content': answer})
            # print(f'chat: {chat}, line 1088 fsdp_workers.py')
        ...
        rm_think_reward = torch.tensor(rm_think_reward, dtype=torch.float32).to(rm_input_ids.device)
        format_valid = torch.tensor(format_valid).to(rm_input_ids.device)

        rm_position_ids = compute_position_id_with_mask(rm_attention_mask)

        rm_inputs = {'input_ids': rm_input_ids, 'attention_mask': rm_attention_mask, 'position_ids': rm_position_ids, 'rm_think_reward': rm_think_reward, 'format_valid': format_valid}
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        is_test = data.meta_info.get('is_test', False)
        add_format_reward_when_wrong = data.meta_info.get('add_format_reward_when_wrong', False)
        ...
        with self.ulysses_sharding_manager:
            ...
            for micro_batch in micro_batches:
                rm_score = self._forward_micro_batch(micro_batch, is_test, add_format_reward_when_wrong)
                    
                output.append(rm_score)

            scores = torch.cat(output, dim=0) # 正常
            
            # print(f"scores shape: {scores.shape}, line 1148 fsdp_workers.py")
            if use_dynamic_bsz:
        ...
        if self.rank == 0:
            print(f'Rank {self.rank} RM scores: {scores}')
            print("==" * 10 + 'end' + "==" * 10)
